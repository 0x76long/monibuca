...
async function testNodeStatus() {
  try {
    ...
    // 检查集群状态
    const response = await fetch(`http://localhost:${HTTP_PORT}/api/cluster/cluster/status`);
    ...
    try {
      ...
      // 获取节点列表进行详细检查
      const nodesResponse = await fetch(`http://localhost:${HTTP_PORT}/api/cluster/nodes`);
...
async function testStreamRegistration() {
  try {
    ...
    // 注册流
    const registerResponse = await fetch(`http://localhost:${HTTP_PORT}/api/cluster/streams`, {
      method: 'POST',
    ...
    // 获取流信息验证
    const getResponse = await fetch(`http://localhost:${HTTP_PORT}/api/cluster/streams/${streamPath}`);
    ...
    // 清理测试数据
    const unregisterResponse = await fetch(`http://localhost:${HTTP_PORT}/api/cluster/streams/${streamPath}`, {
      method: 'DELETE'
    });
