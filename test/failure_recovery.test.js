...
beforeAll(async () => {
  ...
  await waitForServerReady('http://localhost:8090/api/cluster/status', 15000);
  await waitForServerReady('http://localhost:8091/api/cluster/status', 15000);
  await waitForServerReady('http://localhost:8092/api/cluster/status', 15000);
...
describe('Cluster Plugin Failure and Recovery', () => {
  test('Should detect node failure and update cluster state', async () => {
    ...
    let response = await fetch('http://localhost:8090/api/cluster/status');
    ...
    response = await fetch('http://localhost:8090/api/cluster/status');
    ...
    await Page.navigate({ url: 'http://localhost:8090/api/cluster/health' });
  ...
  test('Failover should reassign streams to healthy nodes', async () => {
    ...
    await fetch('http://localhost:8090/api/cluster/streams/register', {
      method: 'POST',
    ...
    const response = await fetch('http://localhost:8090/api/cluster/streams');
  ...
  test('Should handle node recovery', async () => {
    ...
    const response = await fetch('http://localhost:8090/api/cluster/status');
  ...
  test('Manager node failure should trigger leader election', async () => {
    ...
    const response = await fetch('http://localhost:8092/api/cluster/status');
    const status = await response.json();
    ...
    // Check via CDP
    const { Page, Runtime } = cdp;
    await Page.navigate({ url: 'http://localhost:8092/api/cluster/status' });
    await Page.loadEventFired();
