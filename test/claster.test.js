...
describe('Cluster Plugin Core Functionality', () => {
  test('Should connect two nodes in a cluster', async () => {
    ...
    await Page.navigate({ url: 'http://localhost:8090/api/cluster/status' });
  ...
  test('Stream registry synchronization between nodes', async () => {
    ...
    // Register the stream
    const registerResponse = await fetch('http://localhost:8090/api/cluster/streams/register', {
      method: 'POST',
    ...
    const { Page, Runtime } = cdp;
    await Page.navigate({ url: 'http://localhost:8091/api/cluster/streams' });
    await Page.loadEventFired();
  ...
  test('Load balancer directs client to appropriate node', async () => {
    ...
    const { Page, Runtime } = cdp;
    await Page.navigate({ url: 'http://localhost:8090/api/cluster/loadbalancer/test?streamPath=test/stream1' });
    await Page.loadEventFired();
...
describe('Cluster Plugin Performance Tests', () => {
  test('Should handle multiple stream registrations quickly', async () => {
    ...
    for (let i = 0; i < numStreams; i++) {
      ...
      promises.push(
        fetch('http://localhost:8090/api/cluster/streams/register', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(streamData)
        })
