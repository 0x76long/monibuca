...
// Tick is called on each tick of the ticker
func (hm *HealthMonitor) Tick(any) {
	// Check all nodes
	...
	hm.plugin.nodes.Range(func(node *NodeInfo) bool {
		...
		default:
			...
			if node.CurrentLoad.CPUPercent > hm.plugin.Monitoring.AlertThresholds.CPUPercent ||
				...
				// Node is overloaded
				if node.Status != "degraded" {
					hm.plugin.Warn("Node", node.ID, "is degraded (resource overload)")
					node.Status = "degraded"
				}
	...
	for _, nodeID := range nodesToRemove {
		hm.plugin.Info("Removing offline node:", nodeID)
		var found bool
...
func (hm *HealthMonitor) handleOfflineNodeStreams(node *NodeInfo) {
	...
	hm.plugin.Info("Node", node.ID, "is offline with", len(streamsToRedirect), "streams to handle")

	// For each stream, we need to determine what to do
	for _, stream := range streamsToRedirect {
		// Mark the stream as having lost its publisher
		stream.State = "publisher_lost"
		stream.LastUpdated = time.Now()
		hm.plugin.streams.Add(stream)
		...
		if hm.plugin.FailoverDelay > 0 {
			...
			// Just mark the stream as offline
...
func (hm *HealthMonitor) findReplacementSource(stream *StreamInfo) {
	...
	if len(candidateNodes) == 0 {
		hm.plugin.Warn("No healthy nodes have replicas of stream", stream.StreamPath)
		stream.State = "offline"
		hm.plugin.streams.Add(stream)
		return
	}
	...
	hm.plugin.Info("Promoting node", newSourceNode.ID, "as new source for stream", stream.StreamPath)

	// Update stream information
	stream.PublisherNodeID = newSourceNode.ID
	stream.State = "active"
	stream.LastUpdated = time.Now()

	// Remove this node from replication list
	...
	stream.ReplicatedTo = newReplicatedTo
